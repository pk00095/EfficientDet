...
#         regression_batch = np.zeros((batch_size, anchors.shape[0], 9 + 1), dtype=keras.backend.floatx())
#     else:
...
def anchor_targets_bbox(
    ...
    assert image.ndim == 3, f'expected an rgb image, got {image.shape}'

    assert bboxes.ndim == 2, f'bboxes should be of dimension (Nx4) , got {bboxes.shape}'
    assert labels.ndim == 1, f'labels should be of dimension (N,), got {labels.shape}'
    assert bboxes.shape[0] == labels.shape[0], f'num bboxes and labels do not match, {bboxes.shape[0]},{labels.shape[0]}'

    # if detect_quadrangle:
    #     regression_batch = np.zeros((batch_size, anchors.shape[0], 9 + 1), dtype=keras.backend.floatx())
    # else:
    regression_ytrue = np.zeros((anchors.shape[0], 4 + 1), dtype=keras.backend.floatx())
    labels_ytrue = np.zeros((anchors.shape[0], num_classes + 1), dtype=keras.backend.floatx())
    ...
    if bboxes.shape[0]:
        ...
        positive_indices, ignore_indices, argmax_overlaps_inds = compute_gt_annotations(anchors,
                                                                                        ...
                                                                                        negative_overlap,
                                                                                        positive_overlap)
        labels_ytrue[ ignore_indices, -1] = -1
        labels_ytrue[ positive_indices, -1] = 1

        regression_ytrue[ ignore_indices, -1] = -1
        regression_ytrue[ positive_indices, -1] = 1

        # compute target class labels
        labels_ytrue[
             positive_indices, labels[argmax_overlaps_inds[positive_indices]].astype(int)] = 1

        regression_ytrue[ :, :4] = bbox_transform(anchors, bboxes[argmax_overlaps_inds, :])
        # if detect_quadrangle:
        #     regression_ytrue[ :, 4:8] = annotations['alphas'][argmax_overlaps_inds, :]
        #     regression_ytrue[ :, 8] = annotations['ratios'][argmax_overlaps_inds]
    ...
    if image.shape:
        ...
        labels_ytrue[ indices, -1] = -1
        regression_ytrue[ indices, -1] = -1

    return regression_ytrue, labels_ytrue
...
def anchors_for_shape(
    ...
    all_anchors = np.zeros((0, 4), dtype=keras.backend.floatx())
    for idx, p in enumerate(pyramid_levels):
        ...
        shifted_anchors = shift(feature_map_shapes[idx], strides[idx], anchors)
        all_anchors = np.append(all_anchors, shifted_anchors, axis=0)

    return all_anchors.astype(keras.backend.floatx())
