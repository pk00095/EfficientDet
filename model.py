...
if StrictVersion(tf.__version__) <= StrictVersion('2.2.0'):
    ...
    from tensorflow.keras.applications import EfficientNetB3, EfficientNetB4, EfficientNetB5, EfficientNetB6
...
class ClassNet(models.Model):
    def __init__(self, width, depth, num_classes, num_anchors, separable_conv=True, freeze_bn=False, **kwargs):
...
def feature_extractor(model, layer_list):
    features = [model.get_layer(name).output for name in layer_list]
    return features
...
def efficientdet(config, num_classes, freeze_bn=False,
    ...
    box_net = BoxNet(w_head, d_head, num_anchors=config.num_anchors, separable_conv=separable_conv, freeze_bn=freeze_bn,
                     detect_quadrangle=detect_quadrangle, name='box_net')
    class_net = ClassNet(w_head, d_head, num_classes=num_classes, num_anchors=config.num_anchors,
